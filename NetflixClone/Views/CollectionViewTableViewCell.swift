...
class CollectionViewTableViewCell: UITableViewCell {
    ...
    public func configure(with titles: [Title]){
        self.titles = titles
        DispatchQueue.main.sync {[weak self] in  //=======> ask <=======
...
extension CollectionViewTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        APICaller.share.getMovies(with: titleName + " trailer"){ result in
            switch result{
    ...
    func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        let config  = UIContextMenuConfiguration(
            ...
            previewProvider: nil){[weak self] _ in
                let donwloadAction = UIAction(title: "Download", subtitle: nil, image: nil, identifier: nil, discoverabilityTitle: nil, state: .off){ _ in
